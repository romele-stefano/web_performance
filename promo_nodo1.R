...
promo_performance_excel <- read_excel("promoVolantino_nodo1.xlsx")
data <- as.data.frame(promo_performance_excel)
...
outliers <- data[which(data$avg_response > 240000),]
final_data <- data[-which(data$avg_response > 140000), ]
...
plot(final_data$date, final_data$avg_response, main = "Performance promo e volantino - Nodo 1",
...
plot(final_data$id, final_data$avg_response, type = "l", main = "Performance promo e volantino - Nodo 1",
