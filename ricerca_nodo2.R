...
ricerca_performance_excel <- read_excel("ricerca_nodo2.xlsx")
data <- as.data.frame(ricerca_performance_excel)
...
plot(data$date, data$avg_response, main = "Ricerca e navigazione risultati - Nodo 2",
     xlab = "Data", ylab = "Tempo risposta medio (3 ore), in millisecondi")

# extract outliers rows for investivation
outliers <- data[which(data$avg_response > 80000),]
final_data <- data[-which(data$avg_response > 80000), ]
...
plot(final_data$id, final_data$avg_response, type = "l", main = "Ricerca e navigazione risultati - Nodo 2",
